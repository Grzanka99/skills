---
name: fleet
description: Resolve and access the user's personal machines. Use whenever the user refers to archplate, the Arch machine, or the Arch box, including requests that require machine context or access. The host inventory is available only when the user explicitly invokes fleet.
---
...
- `host`: aliases `host machine`, `local machine`, and `this Mac`; read `references/host.md`.
...
2. Use the stored context directly for discussion and planning. When the requested work requires live state or machine action, use local execution for `host` or the record's SSH command for a remote machine. Complete when the user's request is fulfilled on the intended machine.
